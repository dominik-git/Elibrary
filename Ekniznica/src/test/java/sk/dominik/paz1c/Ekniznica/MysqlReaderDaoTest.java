...
class MysqlReaderDaoTest {
	...
	assertEquals(1, DaoFactory.INSTANCE.getReaderDao().getAllReadersByFullName("Lukas").size());
	DaoFactory.INSTANCE.getReaderDao().saveReader(reader);
	pridavaliSme = true;
	assertEquals(1, DaoFactory.INSTANCE.getReaderDao().getAllReadersByFullName("Ja").size());
	assertEquals(DaoFactory.INSTANCE.getReaderDao().getAllReaders().size(),DaoFactory.INSTANCE.getReaderDao().getAllReadersByFullName(" ").size());	
	}
